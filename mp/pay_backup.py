...
def index(request):
    ...
    cursor = connections['default'].cursor()
    cursor.execute("insert into Orders values(null,%s,%s,%s,%s,%s,%s,sysdate(),0,%s,%s,%s)",
                   (my_out_trade_no, bno, sno, hour, need, note, sign, paySign, prepay_id,))
    cursor.close()
...
def notify(request):
    ...
    if request.method == 'POST':
        ...
        jucursor = connections['default'].cursor()
        jucursor.execute(
            "select bno,bneed,otime,sname,prepay_id from Orders,Seller where oid = %s and Orders.sno = Seller.sno",
        ...
        if str(raw[0]['bneed']) != str(dict_data['total_fee']):
            ...
            llcursor.execute("insert into logs values(null,%s,sysdate())",
                             (str(jucursor) + '@' + str(dict_data['total_fee']),))
            llcursor.close()
            return HttpResponse(
        ...
        if ucursor:
            ...
            querystring = {"grant_type": "client_credential", "appid": "wx96cd401177282448",
                           "secret": "3bd6456f8d335d62ce361fe17bbef3c2"}
            headers = {}
            ...
            tmpdata = {"touser": raw[0]['bno'], "template_id": "issFJ6vlxE37dJFehqnQ1xfVa4Ekzm2Hg6mHflWlGk8",
                       ...
                       "data": {"keyword1": {"value": json_serial(raw[0]['otime']), "color": "#000000"},
                                ...
                                "keyword3": {"value": str(int(raw[0]['bneed']) / 100) + "元", "color": "#000000"},
                                "keyword4": {"value": "与" + raw[0]['sname'] + "的约定", "color": "#000000"}}}
